...
const OBHeader = ({screenNumber}) => {
  const { h3, h6, alignCenter, italic } = TextStyles
  let filledMarkers = []
  for(let i=0; i < screenNumber; i++) {
    filledMarkers.push(<View style={{flex: 1, borderWidth: 1, borderColor: Colors.lightgrey2, backgroundColor: Colors.primary}} />)
  }
  let emptyMakers = []
